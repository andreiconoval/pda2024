...
class MyTeamTableViewController: UITableViewController {
    ...
    public var team: CDTeam!
    private var players:[CDPlayer]?
    @IBOutlet weak var Navigation: UINavigationItem!
    let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext

    override func viewDidLoad() {
        super.viewDidLoad()
        players = team.player?.allObjects as? [CDPlayer]
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let action = UIContextualAction(style: .destructive, title: "Delete"){
            ...
            self.team.removeFromPlayer(playerToRemove)
            self.players = self.team.player?.allObjects as? [CDPlayer]
            do {
